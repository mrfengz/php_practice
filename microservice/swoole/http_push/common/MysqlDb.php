...
class MysqlDB
{
    ...
    {
        try {
            $pdo = new PDO(sprintf("%s:host=%s;port=%s;dbname=%s;charset=%s", $config['driver'], $config['host'],  $config['port'], $config['dbname'], $config['charset'])
            ...
            echo $e->getMessage();
            error_log(strval($e), 3, __DIR__ . '/error.log');
            return false;
        }
    }
